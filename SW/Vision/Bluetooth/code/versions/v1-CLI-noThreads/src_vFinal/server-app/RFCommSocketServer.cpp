...
RFCommSocketServer::RFCommSocketServer() : state(sstate::created), cli_sock(-1){
}
...
{
    /* By default, the address structure is initialized to local BT adapter in
RFCommSocket */
    if( !RFCommSocket::isPortValid(port) )
        throw runtime_error("[Socket server]: Invalid port nr. (must be between 1 and 30)");
...
void RFCommSocketServer::Accept()
{
 ...
 */
    ...
    this->rhandle = accept(this->handle, (struct sockaddr *)&cli_addr, &opt);
...
/* Connection accepted: update state */
    this->state = sstate::accepted;
    this->connected = true;
