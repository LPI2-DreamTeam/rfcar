...
void sdp::register_service(const string &service_name, const string &svc_dsc, const string &service_prov)
{
...
void sdp::get_service_info(const string &service_name, struct sockaddr_rc *addr_info)
{
    addr_info = 0;
    int status;
    bdaddr_t target;
    ...
    for (i = 0; i < num_rsp; i++)
    {
        ...
        if (hci_read_remote_name(sock, &(devices+i)->bdaddr, sizeof(name), name, 0) != 0) strcpy(name, "[unknown]");
        ...
        status = sdp_service_search_attr_req(client_session, search_list, SDP_ATTR_REQ_RANGE, attrid_list, &response_list);
        ...
        if(status == 0)
        {
            ...
            for (; r; r = r->next)
            {
                ...
                if(sdp_get_access_protos(rec, &proto_list) == 0 && sdp_get_service_name(rec, get_serv_name, 248) == 0)
                {
                ...
                if(get_serv_name == service_name){
                    service_found = true;
                    addr_info->rc_family = AF_BLUETOOTH;
                    addr_info->rc_channel = port;
                    str2ba(addr, &addr_info->rc_bdaddr);
                    break;
                } 
            }
        }
