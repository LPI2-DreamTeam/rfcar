...
public:
    RFCommSocketServer(int port);
    ~RFCommSocketServer();
   
/**
 * @brief Accepts connections on a socket
 * @return true; if accepted; false, if no connections are available
 * 
 * For non-blocking sockets, it's expected that the server socket is readable;
 *  - at least one connection in pending on the connection queue
 *
 * This function throws an exception for one of two errors:
