...
void SDP::RegisterService(const uint8_t rfcomm_channel, const string &service_name, const string &svc_dsc, const string &service_prov)
{
...
void SDP::SearchService(const string &name, std::vector<sdp_record> &recs)
{
    ...
    for (i = 0; i < num_rsp; i++)
    {
        ...
        if (hci_read_remote_name(sock, &(devices+i)->bdaddr, sizeof(dev_name), dev_name, 0) != 0) strcpy(dev_name, "[unknown]");
