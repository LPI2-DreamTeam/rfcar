...
using ThreadIDIntPair = std::pair<OS::Thread::ID, uint32_t>;
...
static OS::SharedMemory<IntList> g_shared_global_list(&g_global_list);
...
static OS::SharedMemory<ThreadIDIntPairList> g_shared_work_list(&g_work_list);

// Notification signaling that the threahlohd has been reached and the consumer threads may resume work.
static OS::Notification threshold_reached;


bool consume;

void producerFun(OS::Thread* thread) {
...
void consumerFun(OS::Thread* thread) {
...
int main(int argc, char* argv[]) {
	...
	OS::Thread c1("Consumer 1", consumerFun);
	OS::Thread c2("Consumer 2", consumerFun);
...
/////////////////////////////////// TESTING ///////////////////////////////////
	std::map<OS::Thread::ID, std::pair<std::string, uint32_t>> id_name_map;
